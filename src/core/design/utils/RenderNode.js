...
export const RenderNode = ({ render }) => {
    ...
    const { src, isSelected } = useEditor(state => {
        ...
        let src = {};
        if (curNodeId && state.nodes[curNodeId]) {
            try {
                src = query.node(curNodeId).get();
            } catch (error) {
                console.warn('Failed to query selected node:', curNodeId, error);
                src = {};
            }
        }
        return {
                                      ...
                                      <IndicatorDiv
                                          ...
                                          onClick={e => {
                                              setPopOverAnchorEl({
                                                  ...
                                                  position: "top",
                                                  targetNode: src.data?.parent || parent,
                                                  currentNodeId: id
                                              });
                                              if (name === "Text") {
                                                  setProp(props => {
