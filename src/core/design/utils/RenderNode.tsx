...
export const RenderNode = ({ render }) => {
  ...
  const {
    ...
    return {
      ...
      moveable: (node.id && query.node(node.id).isDraggable()) || false,
      deletable: (node.id && query.node(node.id).isDeletable()) || false,
  ...
  const { src, isSelected } = useEditor((state) => {
    ...
    const nodeId = curNodeId instanceof Set ? Array.from(curNodeId)[0] : curNodeId;
      ...
      {(isHover || isActive) && id !== ROOT_NODE
        ? ReactDOM.createPortal(
            <>
                    ...
                    <IndicatorDiv
                      ...
                      onClick={(e) => {
                        setPopOverAnchorEl({
                          ...
                          position: 'top',
                          targetNode: (src as any)?.data?.parent || parent,
                          currentNodeId: id,
                        });
                        if (name === 'Text') {
                          setProp((props) => {
              ...
              <Toolbox
                anchorEl={isSelected && popoverAchorEl ? popoverAchorEl.element : null}
                ...
                onClick={(val) => {
                  addNode({
                    ...val,
                    trg: popoverAchorEl.targetNode,
                    currentNodeId: popoverAchorEl.currentNodeId,
                  });
                  setPopOverAnchorEl(null);
                }}
              />
