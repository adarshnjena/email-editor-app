...
import { Button, Container, Text, Image, Video, HtmlBox, CustomDivider } from '../userComponents';
import {
  TextFields,
  Image as ImageIcon,
  Code,
...
function ContentTab({ onAddComponent }: ContentTabProps) {
  ...
  const buildElement = React.useCallback((comp) => {
    if (comp.isCanvas) {
      return <Element canvas is={comp.component} custom={{ displayName: comp.name }} />;
    }
      ...
      try {
        ...
        if (nodeData && nodeData.data) {
          ...
          selected = {
            ...
            settings:
              state.nodes[currentNodeId].related && state.nodes[currentNodeId].related.settings,
