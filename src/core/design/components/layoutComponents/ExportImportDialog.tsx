...
export function ExportImportDialog({ open, onClose, editorState, htmlContent, onImportSuccess }) {
  ...
  const handleExportEmailTemplate = async () => {
    try {
      ...
      setMessage({
        type: 'error',
        text: 'Failed to export email template: ' + (error?.message || error),
      });
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (validation.valid) {
          ...
          setMessage({
            type: 'success',
            text: `Template imported successfully from ${result.filename}!`,
          });
          setTimeout(() => {
            onClose();
          }, 1500);
      ...
      <DialogContent className={classes.content}>
        {message && (
          <Box className={classes.section}>
            <Alert severity={message.type} onClose={clearMessage} style={{ marginBottom: 16 }}>
              {message.text}
            </Alert>
          </Box>
        )}
        ...
        <Box className={classes.section}>
          ...
          <Paper className={classes.importArea} onClick={loading ? undefined : handleImportJSON}>
