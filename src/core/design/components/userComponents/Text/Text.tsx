...
export function Text({ props, style, parentStyle }) {
  ...
  for (let i = 0; i < x.length; i++) {
    const element = x[i] as HTMLElement;
    element.style.padding = '0px';
    element.style.wordBreak = 'normal';
    element.style.wordWrap = 'normal';
    if (element.firstChild) {
      (element.firstChild as HTMLElement).style.padding = '0px';
    }
  }
  ...
  return (
    ...
    >
      <BraftEditor
        ...
        {...editorConfig(editorState, setEditorState, classes, editorInstance, setProp, theme)}
      />
    </Grid>
  );
}
